#!/usr/bin/env python
# Modified from Adafruit_MCP230XX
...
if __name__ == '__main__':
    ...
    GPIOb = Adafruit_MCP230XX(address = 0x20, num_gpios = 16) # MCP23008
    # mcp = Adafruit_MCP230XX(address = 0x20, num_gpios = 16) # MCP23017


    pin = 8
    GPIOb.setup(pin, GPIOb.OUT)
    
    # Set pin 3 to input with the pullup resistor enabled
    # GPIOb.setup(3, GPIOb.IN)
    # GPIOb.pullup(3, 1)
    ...
    while (True):
        GPIOb.output(pin, 1)  # Pin 0 High
        time.sleep(1)
        GPIOb.output(pin, 0)  # Pin 0 Low
        time.sleep(1)
